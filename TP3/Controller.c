...
int controller_editEmployee(LinkedList* pArrayListEmployee)
{
	...
	if (pArrayListEmployee != NULL && !ll_isEmpty(pArrayListEmployee))
	{
		...
		for (i=0; i<ll_len(pArrayListEmployee); i++)
		{
			...
			{
				employee_printEmployee(aEmployee);
				if (Validate_YesOrNo("esta seguro que desea modificar este empleado? s/n:\n", "ERROR. elija una opcion validad. s/n:\n"))
				{
					do
					{
						printf("SUBMENU DE MODIFICACION\n 1.Nombre\n 2.Horas Trabajadas\n 3.Sueldo\n 4.Salir\n");
						Get_IntMinMax ("ingrese la opcion requerida:\n", "ERROR. ingrese una opcion correcta", 1, 4, &opcion);
	...
	if (pArrayListEmployee != NULL &&  size!= 0)
	{
		...
		if (aEmployee != NULL)
		{
			for (i=0; i<size; i++)
			{
				...
				{
					employee_printEmployee(aEmployee);
					if (Validate_YesOrNo("esta seguro que desea eliminar este empleado? s/n:\n", "ERROR. seleccione una opcion valida. s/n:\n"))
					{
...
int controller_sortEmployee(LinkedList* pArrayListEmployee)
{
	...
	if (pArrayListEmployee != NULL)
	{
		Show_SubMenuOrdenamiento();
		Get_IntMinMax("Seleccione una opcion de ordenamiento: \n", "ERROR. Seleccione una opcion de ordenamiento: \n", 1, 4, &option);
		Get_IntMinMax("Seleccione un criterio de ordenamiento:\n1. Ascendente\n2. Descendente\n", "ERROR. Seleccione un criterio de ordenamiento:\n1. Ascendente\n2. Descendente\n", 1, 2, &criterio);
